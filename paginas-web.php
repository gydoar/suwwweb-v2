<?php include "header.php"; ?>

<div>
	<?php include "parts-body/header-2.php"; ?>
</div>

<!-- Body central -->
 <div class="container table__pricing">
 	<div class="seven columns center">
 		<div class="row">
 			<!-- Tabla Plan Personal -->
 			<div class="six columns table__pricing__personal">
 				<div class="pricing__personal__header">
 					<img width="56px" src="assets/images/table-pricing/icon-plan-personal.png" alt="">
 					<h2>PLAN PERSONAL</h2>
 					<div class="line__pricing__table"><hr></div>
 					<p><span>$ 1.290.000</span></p>
 				</div>

				<ul class="pricing">
					<li><p>5 Páginas internas</p></li>
					<hr>
					<li><p>3 Cuuentas de correo</p></li>
					<hr>
					<li><img width="23px" src="assets/images/table-pricing/icon-x.png" alt=""></li>
					<hr>
					<li><img width="23px" src="assets/images/table-pricing/icon-x.png" alt=""></li>
					<hr>
					<li><img width="23px" src="assets/images/table-pricing/icon-x.png" alt=""></li>
					<hr>
					<li><img width="23px" src="assets/images/table-pricing/icon-x.png" alt=""></li>
					<hr>
					<li><img width="23px" src="assets/images/table-pricing/icon-x.png" alt=""></li>
					<span><hr></span>
					<li><a class="button-blue" href="#">ADQUIRIR</a></li>
				</ul>
 			</div>

 			<!-- Tabla Plan Empresarial -->
 			<div class="six columns table__pricing__empresarial">
 				<div class="pricing__empresarial__header">
 					<img width="63px" src="assets/images/table-pricing/icon-plan-empresarial.png" alt="">
 					<h2>PLAN EMPRESARIAL</h2>
 					<div class="line__pricing__table"><hr></div>
 					<p><span>$ 1.690.000</span></p>
 				</div>

				<ul class="pricing">
					<li><p>10 Páginas internas</p></li>
					<hr>
					<li><p>9 Cuentas de correo</p></li>
					<hr>
					<li><p><img width="24px;" src="assets/images/table-pricing/icon-check-blue.png" alt=""> 3 Imágenes <strong>GRATIS</strong></p></li>
					<hr>
					<li><p>Chat para clientes</p></li>
					<hr>
					<li><p><img width="24px;" src="assets/images/table-pricing/icon-check-blue.png" alt=""> 100% Administrable</p></li>
					<hr>
					<li><p><img width="24px;" src="assets/images/table-pricing/icon-check-blue.png" alt=""> Asesoria personalizada</p></li>
					<hr>
					<li><p>$150.000 en Google Adwords</p></li>
					<span><hr></span>
					<li><a class="button-green" href="#">ADQUIRIR</a></li>
				</ul>
 			</div>
 		</div>
 	</div>
	<!-- Tabla del plan A MEDIDA -->
 		<div class="four offset-by-one columns table__pricing__amedida center">
 				<div class="pricing__amedida__header">
 					<img width="71px" src="assets/images/table-pricing/icon-plan-a-medida.png" alt="">
 					<h2>A MEDIDA</h2>
 					<div class="line__pricing__table"><hr></div>
 					<p><span>A CONVENIR</span></p>
 				</div>
				
				<div class="text__amedida">
					<p>Para aquellas personas que necesitan un plan que se ajuste a sus necesidades de negocio.</p>
					<a class="button-blue" href="#">COTIZAR</a>
					<div class="line__pricing__table__amedida"><hr></div>
					<a href="mailto:info@suwwweb.com">info@suwwweb.com</a>
					<p><a href="#">(57 - 1) 3000 754 / 755</a></p>
				</div>
				
 			</div>
 </div>
<div class="line__divisor"></div>
<!--// Fin tabla de precios -->

<!-- Cacacteristicas todos los planes -->
 <div class="todos__los__planes center">
 	<div class="container">
		<h3><strong>TODOS NUESTROS PLANES INCLUYEN</strong></h3>
		<h4>Esta siempre tranquilo por que todos nuestros planes web incluyen las mejores características.</h4>
		<div class="eight columns offset-by-two line__todos__nuestros__planes"><hr></div>
		<!-- Fila 1 -->
	 	<div class="row items__todos__planes">
	 		<div class="four columns">
	 			<img width="100px;" src="assets/images/todos-los-planes/icon-responsive.png" alt="">
	 			<h5>DISEÑO UNICO Y RESPONSIVE</h5>
	 			<p>Es simplemente el texto de relleno de las imprentas y archivos de texto.</p>
	 		</div>
	 		<div class="four columns">
	 			<img width="100px;" src="assets/images/todos-los-planes/icon-dominio.png" alt="">
	 			<h5>DOMINIO (.COM .NET)</h5>
	 			<p>Es simplemente el texto de relleno de las imprentas y archivos de texto.</p>
	 		</div>
	 		<div class="four columns">
	 			<img width="100px;" src="assets/images/todos-los-planes/icon-hosting.png" alt="">
	 			<h5>ALOJAMIENTO (HOSTING)</h5>
	 			<p>Es simplemente el texto de relleno de las imprentas y archivos de texto.</p>
	 		</div>
	 	</div>

		<!-- Fila 2 -->
	 	<div class="row items__todos__planes margin-top">
	 		<div class="four columns">
	 			<img width="100px;" src="assets/images/todos-los-planes/icon-correos.png" alt="">
	 			<h5>CORREOS CORPORATIVOS</h5>
	 			<p>Es simplemente el texto de relleno de las imprentas y archivos de texto.</p>
	 		</div>
	 		<div class="four columns">
	 			<img width="100px;" src="assets/images/todos-los-planes/icon-sociales.png" alt="">
	 			<h5>INTEGRACIÓN REDES SOCIALES</h5>
	 			<p>Es simplemente el texto de relleno de las imprentas y archivos de texto.</p>
	 		</div>
	 		<div class="four columns">
	 			<img width="100px;" src="assets/images/todos-los-planes/icon-seo.png" alt="">
	 			<h5>POSICIONAMIENTO SEO</h5>
	 			<p>Es simplemente el texto de relleno de las imprentas y archivos de texto.</p>
	 		</div>
	 	</div>

	 	<!-- Fila 3 -->
	 	<div class="">
	 		<ul class="items_small">
	 			<li><p><img src="assets/images/icons/icon-check.png" alt=""> &nbsp;&nbsp;GOOGLE MY BUSINESS</p></li>
	 			<li><p><img src="assets/images/icons/icon-check.png" alt=""> &nbsp;&nbsp;GOOGLE ANALYTICS</p></li>
	 			<li><p><img src="assets/images/icons/icon-check.png" alt=""> &nbsp;&nbsp;GOOGLE MAPS</p></li>
	 		</ul>
	 	</div>
	 </div>
 </div>

 <!-- Caracteristicas como empresa -->
 <div>
	<?php include "parts-body/caracteristicas-empresa.php"; ?>
 </div>
 <!-- // Caracteristicas como empresa -->

 <!-- Call to activon /Llamanos -->
 <div>
   <?php include "parts-body/call-to-action-llamanos.php"; ?>
 </div>
 <!-- // Call to activon /Llamanos -->

 <!-- Encuentra con nosotros -->
 <div>
   <?php include "parts-body/otros-productos.php"; ?>
 </div>
 <!-- // Encuentra con nosotros -->

 <!-- Nuestros Proyectos -->
 <div>
   <?php include "parts-body/nuestros-proyectos-2.php"; ?>
 </div>
 <!-- //Nuestros Proyectos -->

  <!-- footer -->
<?php include "footer.php"; ?>